...
resource "vsphere_virtual_machine" "etcd_nodes" {
  ...
  disk {
    label            = "disk0"
    size             = data.vsphere_virtual_machine.templates[count.index % length(var.hosts)].disks.0.size
    eagerly_scrub    = data.vsphere_virtual_machine.templates[count.index % length(var.hosts)].disks.0.eagerly_scrub
    thin_provisioned = data.vsphere_virtual_machine.templates[count.index % length(var.hosts)].disks.0.thin_provisioned
  }
